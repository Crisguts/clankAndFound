    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/0 backdrop-blur-sm">
      <div className="flex items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              <span className="text-sm font-sans text-muted-foreground hidden md:inline">
                {user.user_metadata?.full_name || user.email}
              </span>
              <Button
                variant="outline"
                onClick={handleSignOut}
                className="rounded-full px-5 border-border hover:border-destructive hover:text-destructive transition-all duration-300 bg-transparent flex items-center gap-2"
              >
